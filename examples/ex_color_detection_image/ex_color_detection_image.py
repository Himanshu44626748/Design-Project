...
def color_detection(images_path):
    images = []
    image_filtered = []
    i = 0
    ...
    for image in images_path:
        ...
        image_filtered.append(my_back_detector.returnFiltered(image, morph_opening=True, blur=True, kernel_size=7, iterations=2))
        fltr = my_back_detector.returnFiltered(image, morph_opening=True, blur=True, kernel_size=7, iterations=2)
    ...
    #images_stack = np.hstack((images,image_filtered)) #The images are stack in order
    #print(images_stack)
    #cv2.imwrite("tiger_filtered.jpg", images_stack) #Save the image if you prefer
        cv2.imwrite(name+'_result.jpg', fltr) #Show the images on screen
    cv2.waitKey(0)
